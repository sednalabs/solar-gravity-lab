/**
 * @name Java/Kotlin trust string literal without recognized enforcement evidence
 * @description Java or Kotlin string literals that claim verification, signing, attestation, approval, sealing, or trust should have nearby validation, digest, identity, provenance, or authorization evidence.
 * @kind problem
 * @problem.severity warning
 * @precision medium
 * @id java/solar/claimed-trust-literal-without-evidence
 * @tags correctness
 *       maintainability
 *       provenance
 *       product-invariants
...
from StringLiteral literal, string claimClass, string missingEvidence
where
  claimText(literal.getValue(), claimClass) and
  contextText(literal.getValue()) and
  missingEvidence(claimClass, missingEvidence) and
  not hasEvidenceFor(literal.getEnclosingCallable(), claimClass)
select literal,
  "This Java/Kotlin string literal makes a trust claim without recognized enforcement evidence in the same callable or a directly called helper. claim_class=" +
