/**
 * @name C/C++ trust string literal without recognized enforcement evidence
 * @description C/C++ string literals that claim verification, signing, attestation, approval, sealing, or trust should have nearby validation, digest, identity, provenance, or authorization evidence.
 * @kind problem
 * @problem.severity warning
 * @precision medium
 * @id cpp/solar/claimed-trust-literal-without-evidence
 * @tags correctness
 *       maintainability
 *       provenance
 *       product-invariants
...
where
  claimText(literal.getValueText(), claimClass) and
  contextText(literal.getValueText()) and
