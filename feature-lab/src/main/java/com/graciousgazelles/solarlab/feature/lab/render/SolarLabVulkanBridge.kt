...
internal object SolarLabVulkanBridge {
    ...
    fun cpuCapabilityDetails(): String =
        if (isLibraryLoaded) nativeGetCpuCapabilityDetails() else "CPU native bridge unavailable"
    ...
    fun hardwareDetails(handle: Long): String =
        if (isLibraryLoaded && handle != 0L) nativeGetHardwareDetails(handle) else "GPU runtime unavailable"
    ...
    private external fun nativeGetCpuCapabilityDetails(): String
    ...
    private external fun nativeGetHardwareDetails(handle: Long): String
}
